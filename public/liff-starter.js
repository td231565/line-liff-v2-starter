window.onload = function () {
  ...
  if (useNodeJS) {
    fetch('/send-id').then(function (res) {
      return res.json()
    }).then(function (data) {
      myLiffId = data.id
...
function initializeLiff(myLiffId) {
  const setting = { liffId: myLiffId }
  liff.init(setting).then(() => {
...
function toggleElement(elementId) {
  ...
  elem.style.display = elem.offsetWidth > 0 && elem.offsetHeight > 0 ? 'none' : 'block'
}
